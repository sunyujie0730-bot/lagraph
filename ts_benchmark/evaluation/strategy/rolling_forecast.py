...
class RollingForecastEvalBatchMaker:
    ...
    def __init__(
        ...
        self.is_st = isinstance(series, np.ndarray)
    ...
    def make_batch_predict(self, batch_size: int, win_size: int) -> dict:
        ...
        if indexes is not None:
            ...
            batch_data["time_stamps"] = time_stamps_batch

        windows = sliding_window_view(
            series, window_shape=(win_size, *series.shape[1:])
        )
        predict_batch = windows[np.array(index_list) - win_size]
        predict_batch = np.squeeze(predict_batch, axis=tuple(range(1, np.ndim(series))))
        batch_data["input"] = predict_batch
    ...
    def make_batch_eval(self, horizon: int) -> dict:
        ...
        if not self.is_st:
            ...
            series = self.series
        horizons = sliding_window_view(
            series, window_shape=(horizon, *series.shape[1:])
        )
        test_batch = horizons[np.array(self.index_list)]
        test_batch = np.squeeze(test_batch, axis=tuple(range(1, np.ndim(series))))
        return {
            "target": test_batch,
        }
...
class RollingForecast(ForecastingStrategy):
    ...
    def _execute(
        ...
        if model.batch_forecast.__annotations__.get("not_implemented_batch"):
            return self._eval_sample(series, covariates, meta_info, model, series_name)
        else:
            return self._eval_batch(series, covariates, meta_info, model, series_name)
    ...
    def _eval_sample(
        ...
        fit_method(train_valid_data, train_ratio_in_tv=train_ratio_in_tv, covariates=covariates)
        ...
        for i, index in itertools.islice(enumerate(index_list), num_rollings):
            ...
            predict = model.forecast(horizon, train, covariates=covariates)
    ...
    def _eval_batch(
        ...
        while predict_batch_maker.has_more_batches():
            start_inference_time = time.time()
            predicts = model.batch_forecast(horizon, predict_batch_maker, covariates=covariates)
            end_inference_time = time.time()
            total_inference_time += end_inference_time - start_inference_time
