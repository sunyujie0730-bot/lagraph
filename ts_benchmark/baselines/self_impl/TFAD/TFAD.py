...
class TFAD:
    ...
    def detect_label(self, test: pd.DataFrame) -> np.ndarray:
        ...
        for i, (input, batch_y) in enumerate(self.thre_loader):
            ...
            # 收集原始数据和"重构数据"用于画图（TFAD没有重构输出，使用输入数据本身）
            ori = torch.mean(input, dim=-1)
            rec1 = torch.mean(input, dim=-1)  # 使用输入数据本身作为重构表示
        ...
        # 画图：原始数据 vs "重构数据"（TFAD没有重构输出，使用输入数据本身）
        ...
        plt.xlabel('time')
        plt.legend()
        plt.savefig(f"/home/zsc/python/CATCH-master/result/figure/TFAD_rec_{self.config.dataset_name}.png")
        plt.clf()
        ...
        plt.xlabel('time')
        plt.legend()
        plt.savefig(f"/home/zsc/python/CATCH-master/result/figure/TFAD_scores_{self.config.dataset_name}.png")
        plt.clf()
