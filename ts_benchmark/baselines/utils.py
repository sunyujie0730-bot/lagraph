...
def anomaly_detection_data_provider(
    ...
    num_workers=32,
    prefetch_factor=8,
    sampler=None,      # 🆕 可选 DistributedSampler（DDP 用）
):
    """
    创建用于异常检测的滑动窗口 DataLoader。

    :param data: 输入数据 (pd.DataFrame)
    :param batch_size: 批次大小
    :param win_size: 窗口大小
    :param step: 窗口滑动步长（内部固定为 1）
    :param mode: 模式 (train/val/test/thre)
    :param num_workers: 数据加载的并行进程数
    :param prefetch_factor: 每个 worker 提前准备的 batch 数
    :param sampler: 可选 DistributedSampler（DDP 使用，优先级高于 shuffle）
    :return: DataLoader 实例
    """
    dataset = SegLoader(data, win_size, 1, mode)

    # 🛡️ DDP 安全：DDP 上下文中强制 num_workers=0 避免多进程冲突
    # DDP 使用多进程（每 GPU 一个进程），DataLoader worker 子进程会
    # 继承 CUDA 上下文导致 fork-bomb 或 CUDA 重初始化错误
    in_ddp = dist.is_available() and dist.is_initialized()
    if in_ddp and num_workers > 0:
        num_workers = 0
    ...
    data_loader = DataLoader(
        ...
        drop_last=(mode == "train"),  # 训练时丢弃不完整 batch
    )
    return data_loader
