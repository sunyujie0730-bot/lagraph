...
class TimeAttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None, n_vars=None, n_tokens=None):
        B, L, _ = queries.shape
        _, S, _ = keys.shape
        H = self.n_heads
        ...
        # Check if inner_attention is TimeAttention (needs n_vars and n_tokens as positional args)
        if isinstance(self.inner_attention, TimeAttention) and n_vars is not None and n_tokens is not None:
            out, attn = self.inner_attention(
                queries,
                keys,
                values,
                attn_mask,
                n_vars,  # positional argument
                n_tokens,  # positional argument
                tau=tau,
                delta=delta
            )
        else:
