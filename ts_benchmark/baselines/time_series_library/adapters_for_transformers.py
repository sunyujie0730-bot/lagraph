...
DEFAULT_TRANSFORMER_BASED_HYPER_PARAMS = {
    ...
    "llm_model": "GPT2",
    "llm_layers": 32,
    "domain_des": "A multivariate time-series benchmark for detecting anomalies in real-world sensor monitoring data."
}
...
class TransformerAdapter(ModelBase):
    ...
    def detect_label(self, test: pd.DataFrame) -> np.ndarray:
        ...
        for i, (batch_x, batch_y) in enumerate(self.thre_loader):
            ...
            # 收集原始数据和重构数据用于画图
            ori = torch.mean(batch_x, dim=-1)
            rec1 = torch.mean(outputs, dim=-1)
        ...
        # 画图：原始数据 vs 重构数据
        plot_len = min(100, len(orin1_energy))
        x = np.arange(plot_len)
        plt.plot(x, orin1_energy[:plot_len], label='original', color='b')
        plt.plot(x, rec3_energy[:plot_len], label='reconstruction', color='r')
        plt.xlabel('time')
        plt.legend()
        plt.savefig(f"/home/zsc/python/CATCH-master/result/figure/{self.model_name}_rec_{self.config.dataset_name}.png")
        plt.clf()
        
        # 画图：异常分数
        plot_len = min(100, len(test_energy))
        y = np.arange(plot_len)
        plt.figure()
        plt.plot(y, test_energy[:plot_len], label='Anomaly Scores', color='b')
        plt.xlabel('time')
        plt.legend()
        plt.savefig(f"/home/zsc/python/CATCH-master/result/figure/{self.model_name}_scores_{self.config.dataset_name}.png")
        plt.clf()
